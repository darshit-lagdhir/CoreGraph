    ...
    async with engine.begin() as conn:
        ...
        def get_metadata(sync_conn):
            ...
            return {
                ...
                "indexes": {
                    t: set(idx["name"] for idx in inspector.get_indexes(t))
        ...
        for table in expected_tables:
            assert (
                table in tables
            ), f"Topological Failure: Table {table} missing from relational vault."
...
async def test_transactional_ddl_integrity():
    """Category D Verification: Asserts that constraints are enforced immediately after migration."""  # noqa: E501
    engine = create_async_engine(settings.DATABASE_URL)
    ...
    # We broaden the search to include the specific asyncpg exception names and generic error markers  # noqa: E501
    assert any(
        x in error_msg for x in ["unique", "duplicate", "23505", "violation"]
    ), f"UNEXPECTED_ERROR: {error_msg}"
