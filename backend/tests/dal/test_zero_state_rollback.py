...
async def test_zero_state_mathematical_consistency(engine, session):
    ...
    result = await session.execute(text("SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name != 'alembic_version'"))
    ...
    result = await session.execute(text("SELECT extname FROM pg_extension WHERE extname IN ('uuid-ossp', 'pg_trgm', 'cube', 'earthdistance')"))
