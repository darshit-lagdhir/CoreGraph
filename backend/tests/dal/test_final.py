...
async def test_global_intelligence_assembly(session):
    ...
    node = Package(name="lodash", ecosystem="npm", version_latest="4.17.21")
    session.add(node)
    await session.commit()

    risk = RiskScoringIndex(package_id=node.id, r_idx=0.85, v_topo=0.9, v_beh=0.4)
    session.add(risk)
    await session.commit()
...
async def test_ai_handoff_serialization(session):
    ...
    # 1. Mock I_Omega
    mock_intel = {"node_metadata": {"name": "lodash"}, "risk_vectors": {"r_idx": 0.85}}
