...
async def test_alert_suppression_logic(session):
    ...
    for _ in range(10):
        await sentinel.trigger_and_persist(
            session, pkg.id, AlertSeverity.CRITICAL, 
            risk=0.95, criticality=0.88, 
...
async def test_emergency_vulnerability_escalation(session):
    ...
    await sentinel.trigger_and_persist(
        session, pkg.id, AlertSeverity.EMERGENCY,
        risk=1.0, criticality=1.0,
    ...
    stmt = select(AlertEvent).where(
        AlertEvent.severity == AlertSeverity.EMERGENCY,
