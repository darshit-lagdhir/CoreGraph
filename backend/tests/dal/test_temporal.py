...
async def test_temporal_existence_range(session):
    ...
    res = await repo.get_package_at_time(session, pkg_name, t_base)
    assert res is not None

    res_future = await repo.get_package_at_time(session, pkg_name, t_base + timedelta(hours=5))
...
async def test_delta_persistence(session):
    repo = TemporalRepository()
    snap = await repo.create_snapshot(session, f"snap-{uuid.uuid4().hex[:4]}")

    id_pkg = uuid.uuid4()
    await repo.record_node_delta(
        session, snap.id, id_pkg, "UPDATED", {"risk_score": {"old": 0.1, "new": 0.9}}
    )
    await session.commit()
