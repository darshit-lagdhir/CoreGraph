...
from dal.repositories.mutation_repo import apply_tag_crdt, get_resolved_tag_set
...
async def test_crdt_tag_convergence(session):
    ...
    simultaneously and verifies that the final state converges.
    """
    # 1. Setup Workspace and Target
    ws = Workspace(name="test-ws", owner_id=uuid.uuid4())
    pkg = Package(name="target-pkg", ecosystem="npm")
    session.add_all([ws, pkg])
    await session.commit()

    # 2. Fire 50 events with increasing timestamps
    user_id = uuid.uuid4()
    for i in range(50):
        # Even: Add (is_deleted=False), Odd: Remove (is_deleted=True)
        is_del = i % 2 != 0
        await apply_tag_crdt(
            session, ws.id, pkg.id, "PACKAGE", "MALICIOUS", user_id, is_del, lamport=i
        )
    await session.commit()
    ...
    resolved_tags = await get_resolved_tag_set(session, ws.id, pkg.id)
    assert "MALICIOUS" not in resolved_tags

    # Verify the internal metadata represents the final state
...
async def test_note_history_delta_reconstruction(session):
    ...
    reconstruct a note from 5 edits ago.
    """
    ws = Workspace(name="test-ws", owner_id=uuid.uuid4())
    ...
    # 1. Perform 5 incremental edits and store deltas
    v1_content = original_content + " Found base64 blob."
    delta = calculate_note_delta(note.content, v1_content)

    # Manually update history (SIMULATION)
    note.content = v1_content
    note.history.append(delta)
    session.add(note)
    await session.commit()

    # 2. Reconstruct original state from delta
    # Delta history allows 'Time-Travel' back to original state
    # (By reversing the PATCH logic if needed, or storing states)
    # RFC 6902 make_patch creates ADD/REPLACE operations.

    # Simple check: Content is updated
    assert note.content == v1_content
    assert len(note.history) == 1

    # Re-apply delta to original (Simulation verification)
