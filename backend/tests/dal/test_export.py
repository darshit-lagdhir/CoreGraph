...
async def test_sbom_compliance_integrity():
    ...
    mock_node = {"name": "test-lib", "version": "1.0.0", "ecosystem": "npm", "sha256_checksum": "a"*64}
...
async def test_bundle_signature_verification():
    ...
    is_valid = await bundler.verify_bundle(compressed, signature, bundler.signing_key.verify_key.encode())
    assert is_valid is True
    
    # 4. Tamper Test
    tampered = payload + b" EXTRA_DATA"
    is_valid_tamper = await bundler.verify_bundle(bundler.compressor.compress(tampered), signature, bundler.signing_key.verify_key.encode())
