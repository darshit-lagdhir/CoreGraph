...
class HealthSweeper:
    ...
    async def sweep_package(self, telemetry_data: Dict[str, Any]) -> VitalityResult:
        ...
        status = "STABLE"
        if s_abd > 0.8: status = "DORMANT"
        elif s_abd > 0.4: status = "AT-RISK"
        if is_bot: status = "COMPROMISED"
        
        latency = (time.perf_counter() - start) * 1000
        print(f"[SWEEP] {telemetry_data['purl']} | S_abd: {s_abd:.2f} | Status: {status} | Latency: {latency:.2f}ms")
...
async def test_ecosystem_health():
    ...
    await sweeper.sweep_package({"purl": "pkg:npm/zombie-lib", "days_inactive": 1, "forced_vitality": True})
