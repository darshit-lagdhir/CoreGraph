...
class HardwareTelemetry(BaseModel):
    ...
    memory_available_mb: float = Field(..., description="Unallocated system RAM under the WSL2 leash")
    vram_used_mb: Optional[float] = Field(None, description="NVIDIA VRAM utilization for clustering GPU acceleration")
...
class HealthResponse(BaseModel):
    ...
    checks: Dict[str, HealthCheckDetail] = Field(..., description="Matrix of dependency-specific audits")
    hardware: HardwareTelemetry = Field(..., description="Hardware-level resource availability summary")
