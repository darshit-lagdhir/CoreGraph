...
@api_router.post(
    "/ingest", response_model=IngestResponseSchema, status_code=status.HTTP_202_ACCEPTED
)
async def trigger_ingestion(request: IngestRequestSchema):
    # JWT & Rate Limit validation simulation boundary context
    result = ingest_ecosystem_structure.apply_async(args=[request.ecosystem, request.name])
