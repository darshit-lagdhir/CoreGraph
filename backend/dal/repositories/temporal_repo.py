...
class TemporalRepository:
    ...
    async def create_snapshot(self, name: str) -> GraphSnapshot:
        snap = GraphSnapshot(name=name)
        self.session.add(snap)
        await self.session.flush()
        return snap

    async def record_node_delta(
        self,
        ...
        delta = NodeDelta(
            snapshot_id=snapshot_id, node_id=node_id, change_type=change_type, diff_payload=diff
        )
