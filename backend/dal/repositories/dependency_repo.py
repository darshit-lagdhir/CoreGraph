...
class DependencyRepository:
    ...
    async def get_local_neighborhood(self, package_id: uuid.UUID, depth: int = 1) -> List[Dict[str, Any]]:
        """
        ...
        stmt_v = select(PackageVersion.id).where(PackageVersion.package_id == package_id).order_by(PackageVersion.sort_key.desc()).limit(1)
        ...
        stmt = (
            ...
            .where(DependencyEdge.parent_version_id == version_id)
        )
