...
async def compute_global_criticality(session: AsyncSession, damping: float = 0.85, max_iter: int = 50, tol: float = 1e-6):
    """
    ...
    for i in range(max_iter):
        ...
        if a_norm > 0: a_next /= a_norm
        if h_norm > 0: h_next /= h_norm
    ...
    vel_res = await session.execute(
        select(MaintainerMetrics.package_id, func.max(MaintainerMetrics.current_velocity))
        .group_by(MaintainerMetrics.package_id)
    )
    ...
    for i, pkg_id in enumerate(pkg_ids):
        ...
        br_res = await session.execute(text("""
...
async def get_top_critical_nodes(session: AsyncSession, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    sql = select(Package, CriticalityScore).join(CriticalityScore).order_by(CriticalityScore.c_idx.desc()).limit(limit)
