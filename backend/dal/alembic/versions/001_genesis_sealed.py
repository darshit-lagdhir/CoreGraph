...
def upgrade() -> None:
    ...
    op.create_table('risk_scoring_index',
        sa.Column('id', sa.UUID(), server_default=sa.text('gen_random_uuid()'), nullable=False),
        sa.Column('package_id', sa.UUID(), nullable=False),
    ...
    """)
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_package_risk_pkg_id ON mv_package_risk_summary (package_id);")
...
def downgrade() -> None:
    ...
    op.execute("DROP TRIGGER IF EXISTS tr_maintainer_metrics_last_modified ON maintainer_metrics CASCADE;")
    ...
    tables = [
        'graph_tags', 'workspaces', 'risk_scoring_index', 'dependency_edges', 
        'package_versions', 'maintainer_metrics', 'packages', 'author_profiles',
        'audit_blocks', 'backup_ledger', 'criticality_scores', 'export_artifacts', 
        'integrity_hashes', 'partitions', 'spatial_index', 'telemetry_logs', 
        'temporal_snapshots', 'tiling_regions'
    ]
    for table in tables:
