...
class DistributedSystemicProfiler:
    ...
    def register_node_execution(self, node_uuid: str, duration_ms: float) -> None:
        ...
        if not self._is_sampling_active:
            ...
            self._local_flame_accumulator["boundary_execution"] = self._local_flame_accumulator.get(
                "boundary_execution", 0
            ) + int(duration_ms)
    ...
    def _simulate_posix_interrupt(self) -> None:
        ...
        if self._is_sampling_active:
            ...
            self._local_flame_accumulator[frame_id] = (
                self._local_flame_accumulator.get(frame_id, 0) + 1
            )
    ...
    async def distill_vitality_vector(self) -> Optional[bytes]:
        ...
        gc_ratio = (
            (self._gc_pause_accumulated_ms / elapsed_epoch_ms) if elapsed_epoch_ms > 0 else 0.0
        )
