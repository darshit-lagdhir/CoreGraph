...
class DistributedTaskTracer:
    ...
    async def inject_trace_context(self, task_signature: Dict[str, Any], parent_span_id: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def enter_span(self, trace_context: Dict[str, Any], priority_weight: float = 1.0, is_anomaly: bool = False) -> Optional[str]:
        """
    ...
    async def finalize_span(self, span_id: str, status: str, result_metadata: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
...
async def _execute_neural_diagnostics() -> None:
    ...
    leaky_metadata = {
        ...
        "nested_creds": {"npm_auth": "npm_xyz123456789012345678901234567890123"}
    }
    ...
    assert final_span["metadata"]["auth_used"] == "[REDACTED_TRACE]", "GitHub Token leaked into Trace Log!"
    assert final_span["metadata"]["nested_creds"]["npm_auth"] == "[REDACTED_TRACE]", "NPM Secret bypassing recursion!"
    ...
    assert 0 <= spans_captured < 30, f"Potato tier failed to truncate spans. Captured: {spans_captured}"
    print(f"    [+] Adaptive Gear-Box engaged. Potato sampling successfully attenuated ({spans_captured}/100 traces captured).")
